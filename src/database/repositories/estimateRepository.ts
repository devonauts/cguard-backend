...
class EstimateRepository {
  ...
  static async create(data, options: IRepositoryOptions) {
    ...
    const record = await options.database.estimate.create(
      {
        ...lodash.pick(data, [
          'estimateNumber',
          'poSoNumber',
  ...
  static async update(id, data, options: IRepositoryOptions) {
    ...
    let record = await options.database.estimate.findOne({
  ...
  static async findById(id, options: IRepositoryOptions) {
    ...
    const record = await options.database.estimate.findOne({
      where: { id, tenantId: currentTenant.id },
      include,
      transaction,
    });
  ...
  static async filterIdsInTenant(ids, options: IRepositoryOptions) {
    ...
    const records = await options.database.estimate.findAll({ attributes: ['id'], where });
  ...
  static async count(filter, options: IRepositoryOptions) {
    const transaction = SequelizeRepository.getTransaction(options);
    const tenant = SequelizeRepository.getCurrentTenant(options);
    return options.database.estimate.count({ where: { ...filter, tenantId: tenant.id }, transaction });
  }
  ...
  static async findAndCountAll({ filter, limit = 0, offset = 0, orderBy = '' }, options: IRepositoryOptions) {
    ...
    if (filter) {
      ...
      if (filter.estimateNumber) whereAnd.push(SequelizeFilterUtils.ilikeIncludes('estimate', 'estimateNumber', filter.estimateNumber));
  ...
  static async findAllAutocomplete(query, limit, options: IRepositoryOptions) {
    ...
    if (query) {
      whereAnd.push({ [Op.or]: [{ ['id']: SequelizeFilterUtils.uuid(query) }, { ['estimateNumber']: { [Op.iLike]: `%${query}%` } }] });
    }
    const where = { [Op.and]: whereAnd };
    const records = await options.database.estimate.findAll({ attributes: ['id', 'estimateNumber'], where, limit: limit ? Number(limit) : undefined, order: [['estimateNumber', 'ASC']] });
    return records.map((record) => ({ id: record.id, label: record.estimateNumber }));
  }
  ...
  static async _createAuditLog(action, record, data, options: IRepositoryOptions) {
    ...
    await AuditLogRepository.log({ entityName: 'estimate', entityId: record.id, action, values }, options);
  }
}
