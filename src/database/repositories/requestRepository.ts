...
class RequestRepository {
  ...
  static async findById(id, options: IRepositoryOptions) {
    ...
    const record = await options.database.request.findOne(
      {
        where: {
          id,
          tenantId: currentTenant.id,
  ...
  static async count(filter, options: IRepositoryOptions) {
    ...
    return options.database.request.count(
      {
        where: {
          ...filter,
          tenantId: tenant.id,
  ...
  static async findAndCountAll(
    ...
    let {
      rows,
      count,
    } = await options.database.request.findAndCountAll({
      where,
      include,
  ...
  static async _createAuditLog(
    ...
    if (data) {
      values = {
        ...
        requestDocumentPDF: data.requestDocumentPDF,
      };
    }
  ...
  static async _fillWithRelationsAndFiles(record, options: IRepositoryOptions) {
    ...
    output.requestDocumentPDF = await FileRepository.fillDownloadUrl(
      await record.getRequestDocumentPDF({
        transaction,
      }),
    );
