...
export default async (req, res, next) => {
  try {
    ...
    // If frontend sent attachment metadata, create attachment records and link to the created note
    if (Array.isArray(data.attachment) && data.attachment.length > 0) {
      try {
        ...
        // Log and continue; do not fail the note creation if attachments fail
