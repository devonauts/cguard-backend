    ...
    const rows = await Promise.all(
      guards.map(async (g: any) => {
        ...
        return {
          ...
          // Prefer live telemetry (pinged while on duty); fall back to clock-in snapshot.
          lastUpdateAt: open ? (open.liveLocationAt || open.punchInTime) : null,
          battery: open ? (toNum(open.liveBattery) ?? toNum(open.punchInBattery) ?? null) : null,
          lat: open ? (toNum(open.liveLatitude) ?? toNum(open.punchInLatitude)) : null,
          lng: open ? (toNum(open.liveLongitude) ?? toNum(open.punchInLongitude)) : null,
