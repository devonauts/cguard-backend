...
export default (router) => {
  ...
  router.post('/sandboxes', async (req, res) => {
    try {
      const { brandName, ownerEmail, ownerFullName } = req.body || {};
      const result = await provisionSandbox(req.database, { brandName, ownerEmail, ownerFullName });
      await writeAudit(req, {
        ...
        statusCode: 200,
        details: { brandName, slug: result.slug },
      });
