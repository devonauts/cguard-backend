...
import TutorialRepository from '../../../src/database/repositories/tutorialRepository';
import VideoTutorialRepository from '../../../src/database/repositories/videoTutorialRepository';
import VideoTutorialCategoryRepository from '../../../src/database/repositories/videoTutorialCategoryRepository';
import CompletionOfTutorialRepository from '../../../src/database/repositories/completionOfTutorialRepository';
...
// ═══════════════════════════ tutorial (legacy scaffold) ═════════════════════
const TUTORIAL_FULL = {
  tutorialName: 'Cómo usar la radio',
  importHash: 'hash-tut-1',
};

describe('crud-g13 · tutorialRepository.create', () => {
  it('persists every field + maps tutorialCategory → tutorialCategoryId + wires tutorialVideos', async () => {
    const db = buildDb();
    await TutorialRepository.create(
      { ...TUTORIAL_FULL, tutorialCategory: 'cat-1', tutorialVideos: ['v-1', 'v-2'] },
      svcOptions(db),
    );

    const written = db.tutorial.calls.create[0];
    for (const [k, v] of Object.entries(TUTORIAL_FULL)) {
      assert.deepStrictEqual(written[k], v, `field "${k}" was dropped or altered on create`);
    }
    assert.strictEqual(written.tutorialCategoryId, 'cat-1', 'tutorialCategory → tutorialCategoryId mapping lost');
    assert.strictEqual(written.tenantId, TENANT);
    assert.strictEqual(written.createdById, USER_ID);
    assert.strictEqual(written.updatedById, USER_ID);

    const row = db.tutorial.rows[0];
    assert.deepStrictEqual(row.__setCalls.tutorialVideos, [['v-1', 'v-2']], 'tutorialVideos relation not written');
  });

  it('a db failure on create REJECTS (no swallowed error)', async () => {
    const db = buildDb();
    db.tutorial.create = async () => {
      throw new Error('DB down');
    };
    await assert.rejects(
      () => TutorialRepository.create({ ...TUTORIAL_FULL }, svcOptions(db)),
      /DB down/,
    );
  });
});

describe('crud-g13 · tutorialRepository.update', () => {
  const seedTutorial = (over: any = {}) => ({
    id: 'tut-1',
    tenantId: TENANT,
    ...TUTORIAL_FULL,
    tutorialCategoryId: 'cat-1',
    ...over,
  });

  it('targets id + tenantId and applies every changed field (incl. category remap)', async () => {
    const db = buildDb({ tutorials: [seedTutorial()] });
    await TutorialRepository.update(
      'tut-1',
      { tutorialName: 'Radio v2', importHash: 'hash-tut-2', tutorialCategory: 'cat-2', tutorialVideos: ['v-9'] },
      svcOptions(db),
    );

    const q = db.tutorial.calls.findOne[0];
    assert.strictEqual(q.where.id, 'tut-1');
    assert.strictEqual(q.where.tenantId, TENANT);

    const row = db.tutorial.rows[0];
    const applied = row.__updateCalls[0];
    assert.strictEqual(applied.tutorialName, 'Radio v2', 'tutorialName silently ignored');
    assert.strictEqual(applied.importHash, 'hash-tut-2', 'importHash silently ignored');
    assert.strictEqual(applied.tutorialCategoryId, 'cat-2', 'category change silently ignored');
    assert.strictEqual(applied.updatedById, USER_ID);
    assert.deepStrictEqual(row.__setCalls.tutorialVideos, [['v-9']]);
  });

  it('throws Error404 for a tutorial in ANOTHER tenant', async () => {
    const db = buildDb({ tutorials: [seedTutorial({ tenantId: OTHER_TENANT })] });
    await assert.rejects(
      () => TutorialRepository.update('tut-1', { tutorialName: 'x' }, svcOptions(db)),
      Error404,
    );
    assert.strictEqual(db.tutorial.rows[0].__updateCalls.length, 0);
  });

  // BUG: src/database/repositories/tutorialRepository.ts:92 — update always
  // writes `tutorialCategoryId: data.tutorialCategory || null` (and
  // setTutorialVideos(data.tutorialVideos || [])), so ANY update payload that
  // omits those fields silently WIPES the stored category and the videos M:N.
  // Not a field-whitelist-safe partial update. Same scaffold defect exists in
  // videoTutorialRepository.update (videoTutorialCategoryId) and
  // completionOfTutorialRepository.update (guardNameId/checkedTutorialId).
  // Latent while the CRM sends the full form, but any partial PUT loses data.
  it.skip('an update payload that omits tutorialCategory KEEPS the stored category', async () => {
    const db = buildDb({ tutorials: [seedTutorial()] });
    await TutorialRepository.update('tut-1', { tutorialName: 'solo renombro' }, svcOptions(db));
    assert.strictEqual(db.tutorial.rows[0].tutorialCategoryId, 'cat-1', 'category wiped by a partial update');
  });
});

describe('crud-g13 · tutorialService (transaction plumbing)', () => {
  it('service.create commits on success and the write reaches the db', async () => {
    const db = buildDb({ videoTutorialCategories: [{ id: 'cat-1', tenantId: TENANT, categoryName: 'Radios' }] });
    const svc = new TutorialService(svcOptions(db));
    await svc.create({ ...TUTORIAL_FULL, tutorialCategory: 'cat-1', tutorialVideos: [] });
    assert.strictEqual(db.tutorial.calls.create.length, 1);
    assert.strictEqual(db.tutorial.calls.create[0].tutorialCategoryId, 'cat-1', 'validated category id lost');
  ...
  it('service.create REJECTS and rolls back when the db write fails (no swallowed error)', async () => {
    ...
    const svc = new TutorialService(svcOptions(db));
    await assert.rejects(() => svc.create({ ...TUTORIAL_FULL }), /insert failed/);
...
// ═══════════════════════════ videoTutorial ══════════════════════════════════
const VIDEO_TUTORIAL_FULL = {
  videoTutorialName: 'Uso del botón de pánico',
  videoTutorialLink: 'https://videos.example.com/panico.mp4',
  importHash: 'hash-vt-1',
};

describe('crud-g13 · videoTutorialRepository.create / update', () => {
  it('create persists every field + maps videoTutorialCategory → videoTutorialCategoryId', async () => {
    const db = buildDb();
    await VideoTutorialRepository.create(
      { ...VIDEO_TUTORIAL_FULL, videoTutorialCategory: 'cat-1' },
      svcOptions(db),
    );
    const written = db.videoTutorial.calls.create[0];
    for (const [k, v] of Object.entries(VIDEO_TUTORIAL_FULL)) {
      assert.deepStrictEqual(written[k], v, `field "${k}" was dropped or altered on create`);
    }
    assert.strictEqual(written.videoTutorialCategoryId, 'cat-1');
    assert.strictEqual(written.tenantId, TENANT);
    assert.strictEqual(written.createdById, USER_ID);
    assert.strictEqual(written.updatedById, USER_ID);
  });
  ...
  it('update targets id + tenantId and applies every changed field', async () => {
    const db = buildDb({
      videoTutorials: [{ id: 'vt-1', tenantId: TENANT, ...VIDEO_TUTORIAL_FULL, videoTutorialCategoryId: 'cat-1' }],
    });
    await VideoTutorialRepository.update(
      'vt-1',
      {
        videoTutorialName: 'Pánico v2',
        videoTutorialLink: 'https://videos.example.com/panico-v2.mp4',
        importHash: 'hash-vt-2',
        videoTutorialCategory: 'cat-2',
      },
      svcOptions(db),
    );
    const q = db.videoTutorial.calls.findOne[0];
    assert.strictEqual(q.where.id, 'vt-1');
    assert.strictEqual(q.where.tenantId, TENANT);
    const applied = db.videoTutorial.rows[0].__updateCalls[0];
    assert.strictEqual(applied.videoTutorialName, 'Pánico v2');
    assert.strictEqual(applied.videoTutorialLink, 'https://videos.example.com/panico-v2.mp4');
    assert.strictEqual(applied.importHash, 'hash-vt-2');
    assert.strictEqual(applied.videoTutorialCategoryId, 'cat-2');
    assert.strictEqual(applied.updatedById, USER_ID);
  });

  it('update throws Error404 for a row in ANOTHER tenant', async () => {
    const db = buildDb({
      videoTutorials: [{ id: 'vt-1', tenantId: OTHER_TENANT, ...VIDEO_TUTORIAL_FULL }],
    });
    await assert.rejects(
      () => VideoTutorialRepository.update('vt-1', { videoTutorialName: 'x' }, svcOptions(db)),
      Error404,
    );
    assert.strictEqual(db.videoTutorial.rows[0].__updateCalls.length, 0);
  });
});

// ═══════════════════════════ videoTutorialCategory ══════════════════════════
describe('crud-g13 · videoTutorialCategoryRepository.create / update', () => {
  it('create persists categoryName + importHash and wires videosInCategory', async () => {
    const db = buildDb();
    await VideoTutorialCategoryRepository.create(
      { categoryName: 'Seguridad física', importHash: 'hash-cat-1', videosInCategory: ['v-1'] },
      svcOptions(db),
    );
    const written = db.videoTutorialCategory.calls.create[0];
    assert.strictEqual(written.categoryName, 'Seguridad física', 'categoryName dropped');
    assert.strictEqual(written.importHash, 'hash-cat-1', 'importHash dropped');
    assert.strictEqual(written.tenantId, TENANT);
    assert.strictEqual(written.createdById, USER_ID);
    const row = db.videoTutorialCategory.rows[0];
    assert.deepStrictEqual(row.__setCalls.videosInCategory, [['v-1']], 'videosInCategory relation not written');
  });

  it('update targets id + tenantId and applies the changed fields', async () => {
    const db = buildDb({
      videoTutorialCategories: [{ id: 'cat-1', tenantId: TENANT, categoryName: 'Vieja', importHash: 'h1' }],
    });
    await VideoTutorialCategoryRepository.update(
      'cat-1',
      { categoryName: 'Renombrada', importHash: 'h2', videosInCategory: ['v-3', 'v-4'] },
      svcOptions(db),
    );
    const q = db.videoTutorialCategory.calls.findOne[0];
    assert.strictEqual(q.where.id, 'cat-1');
    assert.strictEqual(q.where.tenantId, TENANT);
    const row = db.videoTutorialCategory.rows[0];
    const applied = row.__updateCalls[0];
    assert.strictEqual(applied.categoryName, 'Renombrada', 'categoryName silently ignored');
    assert.strictEqual(applied.importHash, 'h2');
    assert.strictEqual(applied.updatedById, USER_ID);
    assert.deepStrictEqual(row.__setCalls.videosInCategory, [['v-3', 'v-4']]);
  });

  it('update throws Error404 for a category in ANOTHER tenant', async () => {
    const db = buildDb({
      videoTutorialCategories: [{ id: 'cat-1', tenantId: OTHER_TENANT, categoryName: 'ajena' }],
    });
    await assert.rejects(
      () => VideoTutorialCategoryRepository.update('cat-1', { categoryName: 'hijack' }, svcOptions(db)),
      Error404,
    );
  });
});
...
const COMPLETION_FULL = {
  dateTutorialStarted: '2026-07-01T08:00:00.000Z',
  tutorialStarted: true,
  wasCompleted: true,
  dateEndedTutorial: '2026-07-02T10:30:00.000Z',
  importHash: 'hash-cot-1',
};

describe('crud-g13 · completionOfTutorialRepository.create / update', () => {
  it('create persists every field + maps guardName/checkedTutorial to *Id columns', async () => {
    const db = buildDb();
    await CompletionOfTutorialRepository.create(
      { ...COMPLETION_FULL, guardName: 'sg-1', checkedTutorial: 'tut-1' },
      svcOptions(db),
    );
    const written = db.completionOfTutorial.calls.create[0];
    for (const [k, v] of Object.entries(COMPLETION_FULL)) {
      assert.deepStrictEqual(written[k], v, `field "${k}" was dropped or altered on create`);
    }
    assert.strictEqual(written.guardNameId, 'sg-1', 'guardName → guardNameId mapping lost');
    assert.strictEqual(written.checkedTutorialId, 'tut-1', 'checkedTutorial → checkedTutorialId mapping lost');
    assert.strictEqual(written.tenantId, TENANT);
    assert.strictEqual(written.createdById, USER_ID);
    assert.strictEqual(written.updatedById, USER_ID);
  });
  ...
  it('update targets id + tenantId and applies every changed field', async () => {
    const db = buildDb({
      completionOfTutorials: [{
        id: 'cot-1', tenantId: TENANT, ...COMPLETION_FULL,
        wasCompleted: false, guardNameId: 'sg-1', checkedTutorialId: 'tut-1',
      }],
    });
    const patch = {
      dateTutorialStarted: '2026-07-03T08:00:00.000Z',
      tutorialStarted: true,
      wasCompleted: true,
      dateEndedTutorial: '2026-07-04T09:00:00.000Z',
      importHash: 'hash-cot-2',
    };
    await CompletionOfTutorialRepository.update(
      'cot-1',
      { ...patch, guardName: 'sg-2', checkedTutorial: 'tut-2' },
      svcOptions(db),
    );
    const q = db.completionOfTutorial.calls.findOne[0];
    assert.strictEqual(q.where.id, 'cot-1');
    assert.strictEqual(q.where.tenantId, TENANT);
    const applied = db.completionOfTutorial.rows[0].__updateCalls[0];
    for (const [k, v] of Object.entries(patch)) {
      assert.deepStrictEqual(applied[k], v, `field "${k}" silently ignored on update`);
    }
    assert.strictEqual(applied.guardNameId, 'sg-2', 'guard change silently ignored');
    assert.strictEqual(applied.checkedTutorialId, 'tut-2', 'tutorial change silently ignored');
    assert.strictEqual(applied.updatedById, USER_ID);
  });

  it('update throws Error404 for a row in ANOTHER tenant', async () => {
    const db = buildDb({
      completionOfTutorials: [{ id: 'cot-1', tenantId: OTHER_TENANT, ...COMPLETION_FULL }],
    });
    await assert.rejects(
      () => CompletionOfTutorialRepository.update('cot-1', { wasCompleted: true }, svcOptions(db)),
      Error404,
    );
  });
});
